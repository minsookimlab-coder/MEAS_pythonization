...
`core/second_channel_worker.py:27` `_PHASE1_TIMEOUT_S = 20 * 60` → `60 * 60`.

이 값은 **고정 대기가 아니라 포기 기준**이다. feedback / threshold_time advance 는
`feedback_poll_interval` 마다 읽어 목표 band 에 들어오면 즉시 다음으로 넘어간다.
상한을 넘기면 '명령이 무시됐나' 를 의심해 **1회 재전송 후 같은 시간을 다시** 기다리고,
그래도 못 가면 `SecondAdvanceTimeout` → 측정 중지 + 알람이다.

초전도 마그넷을 큰 폭으로 움직이면(예: -4 T ↔ 4 T) 램프에만 20분을 넘길 수 있어,
정상 동작인데도 워치독이 측정을 중지시켰다. 총 상한은 재전송 포함 40분 → **2시간**.
Phase 2(안정화) 상한 5분은 그대로다 — 이쪽은 이미 도달한 뒤의 흔들림만 보므로
길게 잡을 이유가 없다.

사용자에게 보이는 '○○분' 문구는 모두 상수에서 파생되므로 함께 바뀐다
(`gui/double_sweep_window.py:1674` 주석 포함).

검증: ① 상수 3600 s ② 도달 시 45 ms 만에 반환(상한과 무관) ③ 미도달 시 재전송
정확히 1회 후 실패 ④ 재전송 안내·실패 메시지가 '60분' 으로 표기.
...
Windows 는 창 이동/크기조절이 시작되면 `WM_ENTERSIZEMOVE` 를 보내고 **OS 자신의
모달 메시지 루프**로 들어간다. 그 동안 Qt 이벤트 루프는 돌지 않아 **포스팅된 이벤트가
전달되지 않는다.** 그런데 이 프로그램의 측정 루프는 그 전달에 의존한다:

    스텝 타이머(GUI) → _sweep_tick → request_step  ──큐잉──▶ 워커 스레드
    워커 VISA 수행 → step_done  ──큐잉──▶ _on_step_done(GUI) → 다음 스텝 예약

즉 창을 끄는 동안 `step_done` 이 GUI 에 닿지 못해 **다음 스텝이 예약되지 않고 측정이
멈춘다.** 놓는 순간 밀린 것이 한꺼번에 처리되면서 '랙' 으로 보인다.

`gui/win_modal_pump.py` 신설 — 모달 루프도 `WM_TIMER` 는 디스패치하므로,
`WM_ENTERSIZEMOVE` 에서 10 ms 네이티브 타이머를 걸고 그 타이머마다
`processEvents(ExcludeUserInputEvents)` 로 밀린 Qt 이벤트를 흘려보낸다.
`WM_EXITSIZEMOVE` 에서 타이머를 없앤다. 사용자 입력은 제외한다(드래그 중 재진입 방지).
Windows 가 아니거나 설치에 실패하면 조용히 아무 일도 하지 않는다.

### [치명] write / measurement 시간이 간헐적으로 튀던 문제

**계측기가 느린 게 아니었다.** 워커가 보고하는 `write`·`measure` 시간은
`perf_counter()` 사이의 값인데, VISA I/O 중에는 GIL 이 풀려 있다가 **타임스탬프를 찍으려
GIL 을 되잡을 때** GUI 스레드가 파이썬 코드를 돌고 있으면 그만큼 기다린다. 그 대기가
계측기 소요 시간으로 기록됐다.

가짜 계측기가 정확히 5 ms(write) / 15 ms(measure 3채널) 걸리도록 고정하고 GUI 부하만
바꿔 가며 워커 보고값을 측정:

| GUI 스레드 부하 | write 보고값 | measure 보고값 | 진짜 소요의 2배 초과 |
|---|---|---|---|
| 없음 | mean 5.19 / max 6.01 | mean 15.68 / max 18.02 | 0 / 0 |
| 20 ms 마다 8 ms | mean 6.49 / max 10.11 | mean 19.96 / max 22.01 | 1 / 0 |
| 50 ms 마다 40 ms | mean 7.38 / **max 25.06** | mean 24.74 / **max 46.88** | **29 / 42** |

→ GUI 부하가 커지면 보고값이 **최대 4배까지** 부풀려진다. 창 드래그·그래프 렌더처럼
GUI 스레드가 바쁜 순간이 그대로 '튀는 시간' 으로 찍힌다.

그래서 GUI 스레드 작업을 줄인다:

- **Graph 창이 보이지 않으면 렌더를 건너뛴다.** 5 fps 렌더 타이머에 가시성 검사가
  전혀 없어서, 창을 닫아 둬도 측정 내내 계속 그리고 있었다. 데이터는 `DataStore` 에
  그대로 쌓이므로 다시 열면 손실 없이 보이고, `showEvent` 에서 밀린 갱신을 한 번에
  반영한다.

**남은 한계(구조적)**: 측정 페이싱이 여전히 GUI 이벤트 루프에 의존한다. 위 펌프는
드래그 구간을 덮지만, GUI 스레드가 오래 파이썬 코드를 돌면 워커가 GIL 을 기다리는 것
자체는 막을 수 없다. 근본 해결은 페이싱을 워커 스레드로 옮기는 것이며(GUI 는 표시만),
`SweepWorker` 와 4개 창의 상태머신을 함께 바꿔야 해서 이번에는 넣지 않았다.

**참고**: 드래그 동작 자체는 실제 화면이 필요해 헤드리스로 검증할 수 없었다. 필터 설치·
메시지 소비·이벤트 배출은 검증했으나, 드래그 중 측정 지속은 실제 환경에서 확인이 필요하다.

---

## v1.07.10 — 2026-09-11 (기능 추가)

### 장비 재연결 버튼

메인 창 빠른 실행 줄에 **[장비 재연결]** 추가. 활성 기기의 세션을 `close()` 한 뒤
다시 `open()` 한다. 우리 쪽 세션이 꼬였을 때 쓰는 수동 수단이다 — 특히 **timeout 은
`_evict_broken()` 이 일부러 세션을 유지**하므로(연결 자체는 살아 있다고 보고), 그
상태를 강제로 풀 방법이 지금까지 Command 창에 `close:<alias>` 를 직접 치는 것뿐이었다.

- 측정 중에는 막는다.
- 결과를 alias 별로 보여 주고, 실패한 항목은 아래의 조치 안내를 그대로 붙인다.
- **다른 프로그램이 잡은 세션은 이 버튼으로 끊을 수 없다.** VISA 에는 남의 세션을
  끊는 수단이 없다 — 그건 장비 쪽 조치(LAN Reset 등)가 필요하며, 안내문에 명시한다.

### 연결 실패 시 원인과 조치를 함께 보여 준다

예전에는 `humanize_error()` 의 한 줄 설명만 떴고, 그마저 200자에서 잘렸다. 현장에서
필요한 건 "그래서 뭘 해야 하나" 인데 그 답이 **인터페이스마다 다르다**.

`core/visa_errors.py` 에 추가:

- `looks_occupied(exc)` — 점유로 볼 수 있는 신호 판정
  (`VI_ERROR_RSRC_BUSY`/`RSRC_LOCKED`, WinError 10048·10061, 관련 문구).
  **timeout 은 넣지 않았다** — 점유일 수도 전원/주소 문제일 수도 있어 단정하면
  엉뚱한 곳을 보게 만든다.
- `connection_failure_hint(exc, alias=, interface_type=, address=, port=)` —
  원인 + 대상 + 인터페이스별 조치를 조립한다.
  - **LAN**: 동시 접속 수 제한 설명, 다른 프로그램 종료, 장비 전면/웹의
    *LAN Reset / Release LAN connections*, 재부팅. 다른 PC 의 세션은 끊을 수 없음을 명시.
  - **USB**: USBTMC 는 한 프로그램만 잡음, NI-MAX 종료, 케이블 재연결.
  - **GPIB**: 보통 공유 가능, 주소 충돌·NI-MAX Reset 확인.
  - **RS232**: 터미널 프로그램 종료, COM 포트 번호 변경 확인.
  - **timeout**: 점유로 단정하지 않고 (a) 전원/주소 (b) 이미 접속됨 두 가능성을 제시.

Connection Test 와 장비 재연결 양쪽이 이 진단을 쓴다. 실패 메시지는 **더 이상 자르지
않는다** — 조치 안내가 잘려 나가면 의미가 없다.

검증: 재연결 시 close→open 순서, 연결 거부 시 LAN 조치 안내, USB 로 바꾸면 USBTMC
안내, timeout 은 두 가능성 제시, Connection Test 도 같은 진단 사용, 측정 중 차단.

---

## v1.07.9 — 2026-09-09 (버그픽스 — 측정 주기)

### [치명] 측정 스텝 주기가 흔들리고 밀리던 문제

"가끔 버벅이다 따닥 하면서 측정되고 Timing 에 음수가 찍힌다" 는 보고의 원인.
(Timing 창의 `idle` 음수 = 한 스텝 왕복이 Time/Point 를 초과했다는 뜻이다.)

**원인 1 — 스텝 타이머가 Qt 기본값(CoarseTimer) 이었다.** `setTimerType` 을 지정한
곳이 없었다. Windows 에서 CoarseTimer 는 간격을 최대 5% 조정하고 **다른 타이머와 묶어
발사(coalescing)** 한다. 이 창에는 렌더 5 fps 타이머와 glow 30 ms 타이머가 함께
돌아서, 스텝이 그쪽에 끌려가 몰려 나갔다.

실측 (목표 주기 50 ms, 그래프 창을 연 채 200스텝):

| | 예전 (CoarseTimer) | 지금 |
|---|---|---|
| 평균 주기 | 61.23 ms | **50.79 ms** |
| 표준편차 | 5.43 ms | **1.79 ms** |
| p95 | 71.17 ms | **55.40 ms** |
| 목표의 120% 초과 | **138 / 199 스텝** | **0 / 199** |
| 199스텝 누적 오차 | **+2235 ms (23% 느림)** | **+158 ms (1.4%)** |

→ `PreciseTimer` 로 지정했다.

**원인 2 — 다음 스텝 예약이 UI 작업 뒤에 있었다.** 기록·그래프·로그·fsync 를 모두
끝낸 다음에야 타이머를 걸어서, 그 사이의 GUI 작업이나 GC 일시정지가 그대로 발사를
늦췄다. 예약을 **UI 작업 앞으로** 옮겼다. 예약 시각은 `t_emit` 기준이라 발사 시각은
같지만, 미리 걸어 두면 뒤따르는 GUI 작업이 주기를 밀지 못한다. 중단·완료 경로에서는
`_on_stop()` 과 `_handle_comm_error()` 가 이 타이머를 멈추므로 안전하다.

**catch-up 은 넣지 않았다.** 늦은 스텝을 즉시 발사해 따라잡으면 그 구간의 실제 sweep
rate 가 설정값보다 빨라진다(스텝당 이동량은 `rate/60 × tpp` 로 고정이므로). 측정
데이터가 왜곡되므로, 늦어진 만큼은 그대로 두고 **알리는 쪽**을 택했다.

### overrun 을 조용히 넘기지 않는다

한 스텝이 Time/Point 를 넘기면 그 구간은 설정한 sweep rate 보다 느리게 진행된 것이다.
예전에는 Idle 라벨이 `max(0, …)` 로 잘려 "overrun" 문구가 **절대 뜨지 않는 죽은
코드**였다. 이제 실제 초과분을 빨간색으로 표시하고, 누적 1·10·100·1000회째에 Sweep
Log 로 알린다.

### 참고 — 함께 확인한 것 (이번에 바꾸지 않음)

- `_on_step_done` 자체는 빠르다 — 4000회 실측 평균 0.45 ms, p99 0.51 ms.
- `_graph_history` 는 sweep 중 무한 누적된다(시작 때만 clear). gen2 GC 일시정지가
  히스토리 크기에 따라 커진다: 10k점 1.1 ms → 200k점 16.5 ms → **500k점 50.1 ms**.
  타이머를 미리 걸도록 바꿔 대부분 흡수되지만, 대기 중에 GC 가 걸리면 그만큼 늦는다.
  장시간·고속 측정에서 문제가 되면 히스토리 상한을 두는 것을 검토할 것.

---

## v1.07.8 — 2026-09-09 (기능 추가)

### 그래프 배치 — 2열 격자 · 드래그 재배치 · 크기 조정

그래프를 추가하면 무조건 아래로 한 줄씩만 쌓였다. 넓은 화면에서 가로 공간이 남는데도
세로로만 길어져 스크롤해야 했다.

- **열 수 선택 (최대 2열)** — 툴바의 `열: 1 / 2`. 2열이면 패널 5개가 `2 + 2 + 1` 로
  배치된다.
- **드래그 앤 드롭으로 위치 변경** — 패널 헤더 왼쪽의 손잡이(`☰`)를 잡아 다른 패널 위에
  놓으면 그 자리에 끼워 넣고 나머지가 밀린다. 패널 전체를 드래그 대상으로 삼으면
  콤보·체크박스·플롯 조작과 충돌하므로 손잡이에서만 시작한다.
- **크기 조정 시 행·열이 함께 움직인다** — 행은 세로 splitter 의 자식이라 높이가 자연히
  같이 움직이고, 열 너비는 한 행의 경계를 끌면 **다른 모든 행에 같은 값이 적용**되어
  열이 어긋나지 않는다 (`_on_row_splitter_moved`, 재귀 방지 플래그 포함).
- 패널의 최소 너비를 **626 px → 486 px** 로 줄였다. X/Y 콤보의 최소 폭(150 px)이
  패널 전체의 최소 너비를 밀어올려, 2열에서 열 경계를 아예 움직일 수 없었다.

### 2D Map 을 별도 창으로 분리

예전에는 Graph 창 오른쪽을 splitter 로 나눠 썼다. 그러면 XY 그래프가 쓸 수 있는 폭이
절반으로 줄고 새 격자 배치와도 겹친다.

- 툴바 `2D Map…` 버튼이 **별도 창**(`MapWindow`)을 연다. 닫아도 숨기기만 하고 다시
  열면 같은 창이 뜬다.
- `MapPanel` 인스턴스는 `GraphWindow` 가 계속 들고 있다 — 그래야 `update_map_base()`
  (Double Sweep 시작 시 호출)가 창을 열지 않고도 동작한다.

검증: 1열/2열 전환 시 행 구성(`[1,1,1,1,1]` ↔ `[2,2,1]`), 첫 행 경계를 끌었을 때 다른
행이 같은 크기로 따라옴(`[1197,699]` 양쪽 동일), 드래그 재배치 후 패널 보존·격자 유지·
열 정렬 유지, 패널 삭제 후 재구성, Map 창 분리와 닫기 동작, 재배치 후에도 그리기 정상.

---

## v1.07.7 — 2026-09-09 (기능 추가)

### 그래프 — 측정 포인트에 마우스를 올리면 값 표시

곡선 위에 마우스를 올리면 가장 가까운 측정 포인트에 원형 마커가 붙고, 그 점의
X·Y 값이 말풍선으로 뜬다. 값은 **화면에 그려진 것과 같은 단위**(SI 접두어 포함)로
쓰므로 축 눈금과 바로 대조된다.
...
- 여러 곡선이 있으면 **가장 가까운 곡선**을 고르고, phase(trace/retrace/dummy)가
  있으면 첫 줄에 그 이름을 적는다.
- **Extra Y 곡선도 잡힌다.** 오른쪽 별도 축을 쓰는 경우 그 축의 좌표계로 따로
  계산하므로, 축마다 접두어가 달라도 맞는 값이 나온다 (예: `0.498 kOhm`).
- 커서에서 14 px 안에 점이 없으면 마커와 말풍선을 감춘다. 플롯 밖으로 나가도 감춘다.

측정 성능에 영향이 없도록:

- 계산은 **마우스가 플롯 위에 있을 때만** 일어난다. `append_point`(측정 루프)와
  렌더 타이머에는 아무것도 더하지 않았다 — 실측 `append_point` 평균 0.005 ms 로 동일.
- 마우스 이동 신호를 `SignalProxy` 로 **초당 30회**로 묶는다. 한 번에 하는 일은 표시
  버퍼(화면 폭 만큼, 보통 1000점 미만)에 대한 numpy 거리 계산 한 번뿐 —
  실측 평균 0.24 ms, 최대 0.34 ms.

**참고**: 표시 버퍼는 오래된 구간을 압축해 보관하므로(기존 동작), 점이 아주 많아지면
마커가 **화면에 실제로 그려진 점**에 붙는다. 원본 데이터의 모든 점에 붙지는 않는다.

---

## v1.07.6 — 2026-09-09 (버그픽스)

### [치명] 멀쩡한 sweep 명령이 '파라미터가 줄었다' 로 잠기던 문제

v1.07.3 에서 넣은 라이브러리 변경 추적의 회귀다. 라이브러리를 저장하면
`write_smua_voltage` · `write_smub_voltage` 처럼 **잘 쓰던 항목이 모든 프로파일에서
비활성화**되고 "다시 등록이 필요" 경고가 떴다.

원인 — `fill_params` 는 sweep 축 placeholder도 **키로 들고 있고, 값이 `[SWEEP]`
표식**이다(Parameter Manager 가 '측정할 때 값이 들어올 자리' 라는 뜻으로 넣는다).
`check_placeholders()` 가 이 키를 '이미 채워진 값' 으로 세는 바람에 비워 둘 자리가
0개로 계산됐고, sweep value 는 1개를 기대하므로 불일치로 판정됐다.

    라이브러리: smua.source.levelv={v}      -> 필요한 자리 {v}
    fill_params: {"v": "[SWEEP]"}           -> 잘못 '채워짐' 으로 계산
    결과: 비워 둘 자리 0 != 기대 1  ->  오탐

같은 실수가 값 치환에도 있었다. 판정을 통과한 항목에서는 축을 `{v}` 로 바꾼 뒤
`fill_params` 를 전부 치환했는데, 여기서 축 키까지 돌면서 명령에 **`[SWEEP]` 이라는
문자열이 그대로 박힐** 수 있었다(그 명령이 장비로 나간다). 이번 오탐 때문에 해당
분기가 실행되지 않아 실제 피해는 없었지만, 같이 고쳤다.

- `filled_keys()` 신설 — 값이 `[SWEEP]` 인 키는 '채워진 값' 에서 제외한다.
  `check_placeholders()` 와 sweep/write 분기의 축 탐지·값 치환이 모두 이걸 쓴다.
- `SWEEP_SENTINEL` 상수로 표식을 한 곳에 모았다.

### needs_fix 를 로드할 때 다시 계산한다

`needs_fix` 는 **파생 상태**인데 프로파일 파일에 저장된다. 위 오탐으로 이미 저장된
잠금이 5개 프로파일에 남아, 로직을 고쳐도 라이브러리를 다시 저장하기 전까지는 항목이
잠긴 채로 보였다.

- `ProfileRegistry.refresh_needs_fix()` 신설 — 라이브러리 기준으로 판정만 다시
  계산한다(명령은 건드리지 않고, 파일도 쓰지 않는다). 라이브러리에 항목이 없으면
  (orphan) placeholder 문제가 아니므로 판정을 비운다.
- `MainWindow._apply_active_profile()` 이 프로파일을 적용할 때마다 호출한다.
  저장된 옛 판정을 믿지 않으므로, 판정 로직이 바뀌어도 다음 실행에서 저절로 맞는다.

검증: 실제 사용자 프로파일 5개를 읽기 전용으로 확인 — 저장된 잠금 2개씩이 재계산 후
모두 0개가 됐다. 헤드리스 회귀 테스트로 ① `{"v": "[SWEEP]"}` 정상 판정 ② 고정
파라미터와 축이 섞인 경우 치환 결과가 `smua.source.levelv={v}` (`[SWEEP]` 미포함)
③ 진짜 파라미터가 늘면 여전히 잡힘 ④ 저장된 잘못된 판정이 로드 시 사라짐
⑤ 메인 창이 잠금 없이 뜸 확인.
