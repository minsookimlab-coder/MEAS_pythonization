...
class InstrumentSettingsUI(QMainWindow):
    ...
    def _discover_drivers(self) -> Dict[str, str]:
        """드라이버 패키지를 훑어 BaseInstrument 하위 클래스를 모두 찾는다.

        반환: {콤보박스 표시 이름: 'pkg.module.ClassName'} — 이 문자열이 그대로
        instruments.yaml 의 class_name 으로 저장되고, InstrumentFactory 가
        importlib 로 되살린다.
        """
        pkg_name = drivers_pkg.__name__
        drivers = {}
        for _, module_name, _ in pkgutil.iter_modules(drivers_pkg.__path__):
            try:
                module = importlib.import_module(f"{pkg_name}.{module_name}")
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if not (issubclass(obj, BaseInstrument) and obj is not BaseInstrument):
                        continue
                    # 드라이버가 다른 모듈에서 import 해온 클래스는 원 소속에서만 등록
                    ...
                    drivers[display_name] = f"{pkg_name}.{module_name}.{name}"
            except Exception as e:
                # 깨진 모듈 하나가 목록 전체를 못 쓰게 만들지 않도록 넘어간다
                print(f"Failed to load driver module '{pkg_name}.{module_name}': {e}")
    ...
    def _on_item_double_clicked(self, item):
        ...
        # 구 레이아웃에서 저장된 경로(driver.m81.… 등)도 현재 경로로 옮겨서 비교해야
        # 콤보박스에서 해당 드라이버가 선택된 상태로 보인다.
        class_name = resolve_class_path(data.get("class_name", ""))
        idx = -1
    ...
    def _add_new_instrument(self):
        ...
        self.instruments_data[new_alias] = {
            "alias": new_alias,
            "class_name": DEFAULT_DRIVER_CLASS_PATH,
            "interface_type": "LAN",
            "address": "192.168.0.1",
            "mac_address": "",
            "port": None,
    ...
    def _test_connection(self):
        ...
        try:
            ...
            from pythonization.instruments.errors import format_error
