...
from gui.mfli_models import (
    MfliAcquireConfig, MfliAuxRead, MfliConfigData, MfliPlotCurveConfig,
    load_mfli_config, save_mfli_config,
)
from gui.vna_models import next_dat_path, next_sweep_folder  # 재사용 (중복 정의 방지)
...
class MfliWindow(QDialog):
    ...
    def _build_ui(self):
        ...
        # Merge with LabOne (core.mfli_merge) — CSV·dat·저장폴더 선택 후 sweep별 병합
    ...
    def _browse_merge_dat(self):
        ...
        # (core.mfli_merge는 폴더/단일 .dat 둘 다 받는다. 단일 파일은 경로를 직접 입력.)
    ...
    def _run_merge(self):
        ...
        try:
            from core.mfli_merge import merge_sweeps
